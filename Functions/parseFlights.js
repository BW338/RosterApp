  ...
  flightTokens.forEach(ft => {
    ...
    if (depTime && arrTime && fullDate) {
      ...
      if (arrMinutes < depMinutes) {
        flights.push({ type:"OP", flightNumber, origin, depTime, destination, arrTime:null, checkin, checkout:null, aircraft, raw:tokens.join(" ") });
        ...
        if (!nextDay) {
          nextDay = { date:null, fullDate:nextDate, flights:[], note:null, tv:null, tsv:null, checkin:null };
          parsedDays.push(nextDay);
        }
        nextDay.flights.push({ type:"OP", flightNumber, origin:destination, depTime:arrTime, destination:null, arrTime:checkout, checkin:null, checkout, aircraft, raw:tokens.join(" ") });
        return;
      }
    }

    flights.push({ type:"OP", flightNumber, origin, depTime, destination, arrTime, checkin, checkout, aircraft, raw:tokens.join(" ") });
  });
